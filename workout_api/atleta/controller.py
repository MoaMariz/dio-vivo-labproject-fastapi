...
async def get_by_name(
    ...
    nome: str = Path(..., title="Nome do Atleta", description="Nome do atleta a ser consultado")) -> AtletaOut:

    atleta = (await db_session.execute(
            select(AtletaModel).filter(AtletaModel.nome == nome))
    ).scalars().first()
...
async def get_by_cpf(
    db_session: DatabaseDependency,
    nome: str = Path(..., title="CPF do Atleta", description="Cpf do atleta a ser consultado")) -> AtletaOut:

    atleta = (await db_session.execute(
            select(AtletaModel).filter(AtletaModel.cpf == cpf))
    ).scalars().first()
